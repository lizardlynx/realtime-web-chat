...
class Listeners {
  ...
  submitButtonPressed(e) {
    ...
    const messageToServer = new TextToServer(messageInput.value, this.destinationUser);
    messageToServer.send(this.socket, this.userID);
    messageInput.value = "";
  }
  ...
  onTextMessage(message) {
    ...
    if (message.idfrom == this.userID) {
      ...
      if (chat === null) {
        ...
        chat = this.selectContact(contact, message.idfrom);
        contactsSection.appendChild(contact);
    
        contact.addEventListener('click', () => {
          this.openedChat = this.contactClick(contact, message.idfrom, message.name);
        });
      }
    }
